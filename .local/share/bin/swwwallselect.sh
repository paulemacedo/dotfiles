...
if [ -z "${monitor_name}" ]; then
  ...
  mon_x_res=$(hyprctl -j monitors | jq --arg name "$monitor_name" '.[] | select(.name==$name) | .width')
  mon_scale=$(hyprctl -j monitors | jq --arg name "$monitor_name" '.[] | select(.name==$name) | .scale' | sed "s/\.//")
fi
...
if [ ! -z "${rofiSel}" ] ; then
    ...
    if [ -n "${monitor_name}" ]; then
        "${scrDir}/swwwallpaper.sh" -s "${setWall}" -m "${monitor_name}"
    else
